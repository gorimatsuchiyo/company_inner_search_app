...
def display_select_mode():
    ...
    with st.sidebar:
        ...
        st.markdown("""
<div style='background-color:#ffffff; padding:10px; border-radius:5px; margin-bottom:20px;'>
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            st.success(f"{main_file_path}（ページNo.{main_page_number}）", icon=icon)  # ← ページ番号も表示
        else:
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice:
                    ...
                    st.info(f"{sub_choice['source']}（ページNo.{sub_choice['page_number']}）", icon=icon)  # ← ページ番号も表示
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # 「ファイルパス」と「ページ番号」
                file_info = f"{file_path}（ページNo.{page_number}）"  # ← ページ番号も表示
            else:
                # 「ファイルパス」のみ
